...
\begin{enumerate}[label=\textbf{(\alph*)}]
\item Design a procedure to do action A with probability $\frac{k}{2^m}$ and 
action B with probability $1-\frac{k}{2^m}$.
\item Show that we can not have a procedure do action A with probability $\frac{1}{\pi}$ and action B with
probability $1-\frac{1}{\pi}$ using only finite number of calls to \texttt{coin()}.
\end{enumerate}
