...
Since $f(0)=1$, and $f(h_o) \ge 1-(1-\frac{1}{n})^n \ge 1-\frac{1}{e}$ we get:
...
Since LHS of that inequality evaluates to $\approx 2$, we can guarantee $S_2 \le 3$
...
containing at least one node. Equivalently, if $H=\min\{i:X_i \le 1\}$ then the height is H.
...
Convince yourself that the output will be \texttt{1 2 3 4 5 6 7}. In-Order Traversal is most commonly seen in 
\texttt{Depth-First Search}, popularly known as \texttt{DFS}. 

\subsection{Binary Search Trees}

The value stored at each node is referred to as its \textbf{key}. A binary search tree (BST) is a 
binary tree in which every non-leaf node satisfies
\begin{itemize}
...
\draw[dashed,thick] (x)--(a);
\draw (x)--(y);
\draw[dashed,thick] (y)--(a);
\draw (y)--(b);

\node[draw=none] at (1,-3.5) {Rotation};
\end{scope}
...
\begin{enumerate}
\item The BST property with respect to keys.
\item The heap property with respect to priorities.
\end{enumerate}
...
The keys satisfy the BST ordering, while every parent has a larger priority than its children.

The priorities are chosen independently at random when nodes are inserted.
...
\begin{enumerate}
\item Insert it as in a standard BST.
\item Assign a random priority.
\item While the heap property is violated, perform rotations to move the new node upward.
\end{enumerate}
...
\begin{enumerate}
\item Repeatedly rotate the node downward until it becomes a leaf.
\item Remove the leaf.
\end{enumerate}
...
\text{Deletion} &= \Theta(\log n),
\end{aligned}
\]

in expectation.

Treaps thus provide a remarkably simple randomized alternative to 
deterministic balanced trees such as AVL trees and red-black trees.
...
insert an element is \(\Theta(1)\) time similar to hash tables.
\subsection{Search}
