...
We wish to schedule as many exams as possible in the same time slot, subject to the constraint 
that no student should have two exams scheduled at the same time\footnote{Or you could just give them a Time Turner and hope for the best :)}.

Equivalently, if two exams share at least one student, they cannot be scheduled together.
...
\subsubsection{A Probabilistic Method Analysis}
...
\subsubsection{A Local Random Decision Method}
...
So we would expect to need to repeat this process about \(e(d+1)\) times to schedule all exams. 
\end{document}